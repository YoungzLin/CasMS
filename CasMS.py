...
from layers import my_GCN,my_TCN, dens_Net, dynamic_routing


class CasMS(torch.nn.Module):
    def __init__(self, args, number_of_features, number_of_nodes):
        super(CasMS, self).__init__()
        self.args = args
    ...
    def _setup_TCN_layers(self):
        self.TCN_layers = my_TCN(tcn_inputsize=self.args.capsule_out_dim,
                                   tcn_hiddensize=self.args.tcn_hiddensize,
                                   tcn_layers=self.args.tcn_layers,
                                   tcn_dropout=self.args.tcn_dropout,
                                   )
    def _setup_dens_layers(self):
        self.dens_layers = dens_Net(dens_inputsize=self.args.tcn_hiddensize,
    ...
    def forward(self, data):
        ...
        graph_representation = self.TCN_layers(sub_graph_representation)
...
class CasMSTrainer(object):
    ...
    def setup_model(self):
        ...
        self.model = CasMS(self.args, self.number_of_features + self.args.number_of_hand_features, self.number_of_nodes)
    ...
    def fit(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            print('CasMS train MSLE loss in ', epoch + 1, ' epoch = ', average_loss)
            print('\n')
    ...
    def evaluation(self, valid_x_batches, valid_y_batches):
        ...
        for step, (valid_x_batch, valid_y_batch) in enumerate(zip(valid_x_batches, valid_y_batches)):
            ...
            losses = losses + loss.item()
            average_loss = losses / (step + 1)
        print('CasMS valid MSLE loss in this epoch = ', average_loss)
        print('\n')
    ...
    def test(self):
        ...
        for step, (test_x_batch, test_y_batch) in enumerate(zip(test_x_batches, test_y_batches)):
            ...
            losses = losses + loss.item()
            average_loss = losses / (step + 1)
        print('CasMS test MSLE loss = ', average_loss)
        print('\n')
